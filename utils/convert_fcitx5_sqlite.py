...
for x in file_list:
    ...
    tables[schema]['NeuboardLocale'] = f"{conf['InputMethod']['LangCode']}_{schema}"
...
# Writing
language_pack = [dict(id=table['NeuboardLocale'], hanShapeBasedKeyCode=table['KeyCode']) for schema, table in tables.items()]
...
for schema, table in tables.items():
    ...
    # put_table(database, table['NeuboardLocale'], table)
