...
import dev.patrickgold.neuboard.app.settings.theme.DisplayKbdAfterDialogs
import dev.patrickgold.neuboard.app.settings.theme.SnyggLevel
...
import dev.patrickgold.neuboard.ime.input.HapticVibrationMode
import dev.patrickgold.neuboard.ime.input.InputFeedbackActivationMode
import dev.patrickgold.neuboard.ime.keyboard.IncognitoMode
import dev.patrickgold.neuboard.ime.keyboard.SpaceBarMode
import dev.patrickgold.neuboard.ime.landscapeinput.LandscapeInputUiMode
import dev.patrickgold.neuboard.ime.media.emoji.EmojiHairStyle
...
import dev.patrickgold.neuboard.ime.smartbar.quickaction.QuickAction
...
import dev.patrickgold.neuboard.ime.text.key.KeyHintConfiguration
...
import dev.patrickgold.neuboard.ime.theme.extCoreTheme
import dev.patrickgold.neuboard.lib.compose.ColorPreferenceSerializer
...
import dev.patrickgold.neuboard.lib.util.VersionName
...
import org.neuboard.lib.color.DEFAULT_GREEN

fun neuboardPreferenceModel() = JetPref.getOrCreatePreferenceModel(AppPrefs::class, ::AppPrefs)

class AppPrefs : PreferenceModel("neuboard-app-prefs") {
    ...
    inner class Clipboard {
        ...
        val syncToNeu = boolean(
            key = "clipboard__sync_to_Neu",
            default = true,
        )
    ...
    inner class Dictionary {
        ...
        val enableNeuboardUserDictionary = boolean(
            key = "suggestion__enable_neuboard_user_dictionary",
            default = true,
        )
    }
    ...
    inner class Theme {
        ...
        val dayThemeId = custom(
            ...
            default = extCoreTheme("neuboard_day"),
            serializer = ExtensionComponentName.Serializer,
        )
        val nightThemeId = custom(
            key = "theme__night_theme_id",
            default = extCoreTheme("neuboard_night"),
            serializer = ExtensionComponentName.Serializer,
        )
        val accentColor = custom(
        ...
        return when (entry.key) {
            ...
            "media__emoji_recently_used" -> {
                ...
                val recent = emojiValues.map {
                    dev.patrickgold.neuboard.ime.media.emoji.Emoji(it, "", emptyList())
                }
