...
package dev.patrickgold.neuboard.app.devtools
...
import dev.patrickgold.neuboard.ime.keyboard.DebugLayoutComputationResult
...
import org.neuboard.lib.snygg.SnyggMissingSchemaException
    ...
    DevtoolsOverlayBox(title = "Failed to load stylesheet, fell back to base style") {
        ...
        if (cause is SnyggMissingSchemaException) {
            DevtoolsSubGroup(title = "Explanation") {
                DevtoolsText(
                    text = """
                    It appears you’re trying to load a theme designed for neuboard v0.4 (Snygg v1), which isn’t compatible with the latest release using Snygg v2.
