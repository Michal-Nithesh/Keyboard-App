...
import dev.patrickgold.neuboard.ime.input.InputKeyEventReceiver
...
import dev.patrickgold.neuboard.ime.nlp.PunctuationRule
import dev.patrickgold.neuboard.ime.nlp.SuggestionCandidate
import dev.patrickgold.neuboard.ime.onehanded.OneHandedMode
import dev.patrickgold.neuboard.ime.popup.PopupMappingComponent
import dev.patrickgold.neuboard.ime.text.composing.Composer
...
import dev.patrickgold.neuboard.ime.text.keyboard.TextKeyboardCache
...
import dev.patrickgold.neuboard.lib.titlecase
import dev.patrickgold.neuboard.lib.uppercase
...
import org.neuboard.lib.kotlin.collectIn
...
class KeyboardManager(context: Context) : InputKeyEventReceiver {
    ...
    private fun handleLanguageSwitch() {
        when (prefs.keyboard.utilityKeyAction.get()) {
            ...
            else -> NeuboardImeService.switchToNextInputMethod()
        }
    }
    ...
    override fun onInputKeyUp(data: KeyData) = activeState.batchEdit {
        when (data.code) {
            ...
            KeyCode.IME_SHOW_UI -> NeuboardImeService.showUi()
            KeyCode.IME_HIDE_UI -> NeuboardImeService.hideUi()
            KeyCode.VOICE_INPUT -> NeuboardImeService.switchToVoiceInputMethod()
            KeyCode.SETTINGS -> NeuboardImeService.launchSettings()
            KeyCode.SYSTEM_PREV_INPUT_METHOD -> NeuboardImeService.switchToPrevInputMethod()
            KeyCode.SYSTEM_NEXT_INPUT_METHOD -> NeuboardImeService.switchToNextInputMethod()
    ...
    override fun onInputKeyRepeat(data: KeyData) {
        NeuboardImeService.inputFeedbackController()?.keyRepeatedAction(data)
        when (data.code) {
            KeyCode.ARROW_DOWN,
            KeyCode.ARROW_LEFT,
            KeyCode.ARROW_RIGHT,
